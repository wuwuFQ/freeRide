// pages/publishPepole/publishPepole.js
...
Page({
  ...
  numberPepoleInputChange(event) {
    this.data.numPepole = event.detail;
  },
  ...
  pushBtnHandle() {
    ...
    if (that.data.isModify) {
      // 修改行程
      wx.request({
        url: app.globalData.httpURL + '/release/updateTripPeopleInfo',
        method: "POST",
        data: {
        ...
        success: res => {
          ...
          if (response.code == 10000) {
            WXNotify.postNotificationName(WXNotify.wx_modifyPeopleInfo);
            wx.showToast({
              title: '行程修改成功',
              icon: 'success',
              duration: 1500,
      ...
      // 发布行程
    wx.request({
      url: app.globalData.httpURL + '/release/saveTripPeopleInfo',
      method: "POST",
      data: {
        "userId": app.globalData.userInfo.id,
        "startProvince": that.data.region_start[0],
